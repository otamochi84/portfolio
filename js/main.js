        ...
        function setupContent() {
            ...
            // ここで <br class="br-mobile"> を挿入していたのを削除し、半角スペースで繋ぐように変更
            // 名前を一文字ずつspanで囲み、アニメーション対象にする
            nameEl.innerHTML = portfolioConfig.name.split(' ').map(word =>
                `<span class="reveal-text"><span>${word}</span></span>`
            ).join(' ');

            // navのブランド名にもセット
            document.getElementById('nav-role-text').innerText = portfolioConfig.role;
            document.getElementById('user-bio').innerText = portfolioConfig.bio;
            document.getElementById('copyright').innerHTML = `&copy; ${new Date().getFullYear()} ${portfolioConfig.role}`;
            document.getElementById('free-text-title').innerText = portfolioConfig.freeTextTitle;
            document.getElementById('free-text-body-en').innerText = portfolioConfig.freeTextBodyEn;
            document.getElementById('free-text-body-jp').innerText = portfolioConfig.freeTextBodyJp;
        ...
        function startOpeningAnimation() {
            ...
            tl
                ...
                .add({
                    ...
                    opacity: [0, 1],
                    duration: 800,
                    delay: anime.stagger(100), // 1行ずつずらす
                    easing: 'easeOutQuad'
                }, '-=600')
                // 7. スクロール指示
                .add({
                    ...
                    duration: 800,
                    complete: function () {
                        // 出現後にバウンドアニメーションを有効にするためスタイルリセット
                        document.querySelector('.scroll-indicator').style.animation = 'bounce-slow 2s infinite';
                    }
                }, '-=400')
                .finished.then(() => {
        ...
        function initCursor() {
            ...
            function animateCursor() {
                const lerpFactor = 0.15; // 少し滑らかに追従
        ...
        window.closeModal = function () { // グローバルスコープに公開
            anime.timeline({
                easing: 'easeInQuad',
                complete: () => {
